...
class InViewBuilder {
    ...
    getInview(id) {
        var self = this;
        self.db.query("SELECT inview_user_id AS id, email, updated_date AS lastupdate FROM inview_users_r WHERE inview_user_id = $1", [id], function (err, results) {
            if (err) {
    ...
    getInviewCampaigns() {
        ...
        self.db.query(query, [self.inview.id], function (err, results) {
            if (err) {
                self.handler(err, {});
                return;
            }
    ...
    getSurveyItems() {
        ...
        self.db.query(query, [playlist.id, self.iid], function (err, results) {
            if (err) {
                self.handler(err, self.inview);
            }
            else {
...
exports.InViewBuilder = InViewBuilder;
//# sourceMappingURL=InViewBuilder.js.map