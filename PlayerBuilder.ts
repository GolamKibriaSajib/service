...
export class PlayerBuilder {
    ...
    resUriExists(uri: string) {
        return this.resmap[uri] !== undefined;
    }

    resExists(res: iResource) {
        return this.resmap[res.u] !== undefined;
    }

    resAdded(res: iResource) {
    ...
    mkRes(u: string, s?: number, v?: string): iResource {
        var r: any = {};
        r.u = u;
        if (s) {
            r.s = s;
    ...
    build(tenant: number, pid: number, db: any, handler: (err, result) => void) {
        var self = this;
        self.tenant = tenant;
        self.db = db;
    ...
    // Get the player
    getPlayer(pid: number): void {
        var self = this;
        self.db.query("SELECT player_id, player_name, preload, preload_days, preload_start_time, preload_duration FROM player_r where player_id = $1", [pid], function (err: any, results: any) {
            if (err) {
                self.handler(err, {});
                return;
            }
    ...
    getPlayerPlaylists() {
        ...
        self.db.query(query, [self.player.playerID], function (err: any, results: any) {
            if (err) {
                self.handler(err, {});
                return;
            }
    ...
    getLayoutResources(depth: number, layoutList: number[], callback: (err) => void) {
        ...
        self.db.query('SELECT url, type from layout_resources_r where layout_id IN (' + list + ');', [], function (err: any, results: any) {
            if (err) {
                callback(err);
            } else {
                if (results && results.rows && (results.rows.length > 0)) {
    ...
    getLayoutItems() {
        ...
        self.db.query(query, [playlist.id, ContentType.Layout], function (err: any, results: any) {
            if (err) {
                self.handler(err, self.player);
            } else {
                if (results && results.rows && (results.rows.length > 0)) {
    ...
    getMediaItems() {
        ...
        self.db.query(query, [playlist.id, ContentType.Image, ContentType.Video], function (err: any, results: any) {
            if (err) {
                self.handler(err, self.player);
            } else {
                if (results && results.rows && (results.rows.length > 0)) {
